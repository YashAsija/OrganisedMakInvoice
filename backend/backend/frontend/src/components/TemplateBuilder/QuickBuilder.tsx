...
export default function QuickBuilder({ onSave, onCancel, switchToAdvanced }: QuickBuilderProps) {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-[calc(100dvh-120px)] lg:h-[calc(100vh-120px)] bg-white dark:bg-[#111a36] border border-[#bae6fd]/60 dark:border-[#223269]/60 rounded-3xl overflow-hidden shadow-sm animate-in fade-in duration-200 w-full">
      ...
      <div className="w-full lg:w-[420px] shrink-0 bg-white dark:bg-[#111a36] border-b lg:border-b-0 lg:border-r border-[#bae6fd]/40 dark:border-[#223269]/40 flex flex-col min-h-[45dvh] sm:min-h-[50dvh] lg:h-full z-10">
        ...
        <div className="p-4 sm:p-6 border-b border-[#bae6fd]/30 dark:border-[#223269]/30 flex items-center justify-between bg-[#f4f9ff] dark:bg-[#0b1329]/40">
           <div>
             ...
             <p className="text-[10px] text-[#64748b]/80 dark:text-zinc-400 mt-0.5">
               Step <span className="text-[#0f172a] dark:text-white font-extrabold">{currentStep}</span> of {STEPS.length}: <span className="text-[#0284c7] dark:text-[#38bdf8] font-extrabold">{STEPS[currentStep-1].title}</span>
             </p>
           </div>
           <button 
             onClick={onCancel} 
             className="text-[10px] font-black text-[#64748b] hover:text-[#0284c7] dark:text-zinc-500 dark:hover:text-[#38bdf8] uppercase tracking-widest cursor-pointer"
           >
             Cancel
           </button>
         </div>
        ...
        <div className="w-full bg-[#bae6fd]/20 dark:bg-zinc-800 h-1 relative overflow-hidden">
          {(() => {
            ...
            return (
              <div 
                className={`bg-gradient-to-r ${stepGradients[currentStep - 1] || 'from-[#0284c7] to-[#2563eb]'} h-1 transition-all duration-500 ease-out`} 
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
            ...
            <div className="space-y-4 animate-in fade-in duration-200">
              ...
              <div className="grid grid-cols-2 gap-3">
                {['Invoice', 'Estimate', 'Proforma', 'Credit Note', 'Purchases', 'Purchase Order', 'Purchase Debit Note'].map((type, idx) => {
                  ...
                  const textColors = ['text-[#0284c7] dark:text-[#38bdf8]', 'text-sky-500', 'text-blue-500', 'text-indigo-500', 'text-violet-500', 'text-emerald-500', 'text-teal-500'];
                  return (
                    <button
                      key={type}
                      ...
                      className={`p-4 rounded-2xl border text-left transition-all cursor-pointer ${
                        active 
                          ? `border-[#0284c7] dark:border-[#38bdf8] bg-[#f4f9ff] dark:bg-[#0b1329] shadow-xs ring-1 ring-[#0284c7]/20` 
                          : 'border-[#bae6fd]/60 dark:border-[#223269]/60 hover:border-[#0284c7]/40 hover:bg-[#f4f9ff]/20'
                      }`}
                    >
                      <div className="flex justify-between items-center mb-3">
                         <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${active ? 'bg-white dark:bg-[#111a36] border border-[#bae6fd]/60 shadow-2xs' : 'text-[#64748b]'}`}>
          ...
          {currentStep === 2 && (
            <div className="space-y-4 animate-in fade-in duration-200">
              <h3 className="text-xs font-black text-[#0f172a] dark:text-white uppercase tracking-wide" style={{ fontFamily: "'Fraunces', serif" }}>Base Template Style</h3>
              <div className="grid grid-cols-2 gap-3">
                {['Modern', 'Corporate', 'Minimal', 'Premium'].map((style, idx) => {
                  const active = state.templateStyle === style;
                  const textColors = ['text-[#0284c7] dark:text-[#38bdf8]', 'text-sky-500', 'text-blue-500', 'text-indigo-500'];
                  return (
                    <button
                      key={style}
          ...
          {currentStep === 3 && (
            <div className="space-y-4 animate-in fade-in duration-200">
              <h3 className="text-xs font-black text-[#0f172a] dark:text-white uppercase tracking-wide" style={{ fontFamily: "'Fraunces', serif" }}>Brand Customization</h3>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <label className="flex items-center gap-3 p-3 bg-[#f4f9ff] dark:bg-[#0b1329] border border-[#bae6fd]/60 dark:border-[#223269]/60 rounded-xl cursor-pointer hover:bg-[#e0f2fe]/45 transition-all">
                    <input type="checkbox" checked={state.branding.showLogo} onChange={e => setState({...state, branding: {...state.branding, showLogo: e.target.checked}})} className="w-4 h-4 rounded border-[#bae6fd] text-[#0284c7] dark:text-[#38bdf8] focus:ring-[#0284c7] cursor-pointer" />
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input type="color" value={state.branding.primaryColor} onChange={e => setState({...state, branding: {...state.branding, primaryColor: e.target.value}})} className="w-10 h-10 rounded-xl cursor-pointer border border-[#bae6fd]/60 p-0" />
                    <input type="text" value={state.branding.primaryColor} onChange={e => setState({...state, branding: {...state.branding, primaryColor: e.target.value}})} className="flex-1 p-2 bg-white dark:bg-[#111a36] border border-[#bae6fd]/60 dark:border-[#223269]/60 rounded-xl text-[11px] font-mono uppercase focus:outline-none focus:border-[#0284c7] dark:text-white" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <select value={state.branding.fontFamily} onChange={e => setState({...state, branding: {...state.branding, fontFamily: e.target.value}})} className="w-full p-2.5 bg-white dark:bg-[#111a36] border border-[#bae6fd]/60 dark:border-[#223269]/60 rounded-xl text-xs font-bold text-[#0f172a] dark:text-zinc-200 focus:outline-none focus:border-[#0284c7] cursor-pointer">
                ...
                <div>
                  ...
                  <div className="flex bg-[#f4f9ff] dark:bg-[#0b1329] p-1 rounded-xl">
                    {['Left', 'Center', 'Right'].map(align => (
                       <button 
                         key={align} 
                         type="button"
                         onClick={() => setState({...state, branding: {...state.branding, titleAlignment: align as any}})}
                         className={`flex-1 py-1.5 text-[10px] font-black uppercase tracking-wider rounded-lg transition-all cursor-pointer ${state.branding.titleAlignment === align ? 'bg-white dark:bg-[#111a36] shadow-xs text-[#0284c7] dark:text-[#38bdf8]' : 'text-[#64748b]/80 hover:text-[#0284c7]'}`}
                       >
                         {align}
                       </button>
                    ))}
          ...
          {currentStep === 4 && (
            <div className="space-y-4 animate-in fade-in duration-200">
              <div className="flex flex-col">
                <h3 className="text-xs font-black text-[#0f172a] dark:text-white uppercase tracking-wide" style={{ fontFamily: "'Fraunces', serif" }}>Selectable Sections</h3>
              ...
              <div className="grid grid-cols-1 gap-2 max-h-[260px] sm:max-h-[300px] overflow-y-auto pr-1">
                {[
                  ...
                  return (
                    <label key={key} className="flex items-center justify-between p-3 bg-[#f4f9ff]/50 dark:bg-[#0b1329]/50 border border-[#bae6fd]/60 dark:border-[#223269]/60 rounded-xl cursor-pointer hover:bg-[#e0f2fe]/45 transition-colors">
                      ...
                      <input 
                        ...
                        className="w-4 h-4 rounded border-[#bae6fd] text-[#0284c7] dark:text-[#38bdf8] focus:ring-[#0284c7] cursor-pointer" 
                      />
                    </label>
                  );
                })}
          ...
          {currentStep === 5 && (
            <div className="space-y-4 animate-in fade-in duration-200">
              <h3 className="text-xs font-black text-[#0f172a] dark:text-white uppercase tracking-wide" style={{ fontFamily: "'Fraunces', serif" }}>Itemized Grid Layout</h3>
          ...
          {currentStep === 6 && (
            <div className="space-y-4 animate-in fade-in duration-200">
              <div className="text-center py-4">
                 ...
                 <h3 className="text-sm font-black text-[#0f172a] dark:text-white uppercase tracking-wider mb-2" style={{ fontFamily: "'Fraunces', serif" }}>Setup is Ready!</h3>
                 ...
                 <div className="space-y-2">
                    <button 
                      ...
                      className="w-full py-2.5 bg-[#0284c7] dark:bg-[#38bdf8] border border-[#0369a1] dark:border-[#0284c7] hover:bg-[#0369a1] dark:hover:bg-[#0284c7] text-white dark:text-[#0b1329] rounded-xl text-[10px] font-black uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all shadow-sm cursor-pointer hover:-translate-y-px active:scale-[0.98]"
                    >
                      <Save className="w-4 h-4" /> Save Template
                    </button>
                    <button 
                      onClick={() => switchToAdvanced(template)} 
                      className="w-full py-2.5 bg-[#f4f9ff] hover:bg-[#e0f2fe] text-[#0284c7] dark:text-[#38bdf8] dark:bg-[#1b264f]/45 dark:hover:bg-[#1b264f] border border-[#bae6fd] dark:border-[#223269] rounded-xl text-[10px] font-black uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all cursor-pointer hover:-translate-y-px active:scale-[0.98]"
                    >
        ...
        <div className="p-4 border-t border-[#bae6fd]/30 dark:border-[#223269]/30 flex items-center justify-between bg-[#f4f9ff] dark:bg-[#0b1329]/40 mt-auto">
          <button 
            ...
            className={`px-3 py-2 flex items-center gap-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-colors cursor-pointer ${currentStep === 1 ? 'text-zinc-350 dark:text-zinc-700 cursor-not-allowed' : 'text-[#64748b] hover:bg-[#e0f2fe] dark:hover:bg-[#1b264f]'}`}
          >
          ...
          {currentStep < STEPS.length ? (
            <button 
              onClick={handleNext} 
              className="px-4 py-2 bg-[#0284c7] dark:bg-[#38bdf8] text-white dark:text-[#0b1329] border border-[#0369a1] dark:border-[#0284c7] hover:bg-[#0369a1] dark:hover:bg-[#0284c7] flex items-center gap-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all cursor-pointer shadow-sm"
            >
      ...
      <div className="flex-1 relative flex flex-col h-full min-h-[60vw] sm:min-h-0 overflow-hidden bg-[#f4f9ff]/40 dark:bg-[#0b1329]/20">
        <div 
